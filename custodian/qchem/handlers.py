...
class QChemErrorHandler(ErrorHandler):
    ...
    Errors involved in optimization are handled separately.
    """
...
class QChemOptErrorHandler(ErrorHandler):
    """
    An ErrorHandler class that handles errors in optimization calculations.
    ...
    def correct(self):
        ...
        if "out_of_opt_cycles" in self.errors:
            ...
            # You should never get here. If you get here, you should be using
            # QChemErrorHandler in addition to this class
            print("Errors:", self.errors)
            print("Must have gotten an error which is correctly parsed but not included in the handler. FIX!!!")
            return {"errors": self.errors, "actions": None}
